...
The proof uses Morrey's estimate from Chapter 4. Write $f$ as its average plus an integral of the gradient, apply Poincar\'e on balls, and Lebesgue differentiation pins down the base point. The exponent $p^*$ is sharp. You don't get $L^q$ for $q > p^*$.

\subsection{Approximate differentiability}\label{subsec:ap-diff}

This is a weird object at first. The approximate derivative says: $f$ looks differentiable if you ignore a set of density zero.
...
You replace the ordinary limit with an approximate limit. The set where the difference quotient is large could be nonempty, even uncountable. You just need it to be thin in a density sense.
...
When $p > n$ you jump all the way to classical differentiability. This is where the Sobolev embedding from Chapter 4 really pays off.
...
Short and clean. Morrey does the work.
...
Two lines. The first proof back in Chapter 3 was this painful argument with directional derivatives and Fubini and countable dense sets and covering arguments. This proof hides all that pain inside the Chapter 4 prerequisites. I don't care. Two lines.
...
Convexity is a global geometric condition but it forces local analytic regularity. A convex function can't oscillate too wildly --- it's hemmed in by its supporting hyperplanes. Combined with Rademacher: every convex function is differentiable a.e. But Aleksandrov goes one derivative further.

\subsection{Aleksandrov's theorem}\label{subsec:aleksandrov}

This result is amazing. A convex function has second derivatives almost everywhere. No smoothness assumed.
...
Think about what this says. The second-order Taylor remainder, averaged over a ball, is $o(r^2)$. Convex functions have a second-order Taylor expansion a.e., and the Hessian is nonneg. You'd \emph{expect} that from convexity, sure, but proving the Hessian exists without any smoothness assumption is the hard part.
...
Look, $f(x) = |x|$ on $\R$ has $f''(x) = 0$ for $x \neq 0$ and isn't twice differentiable at $x = 0$. But that's a single point, measure zero. Aleksandrov says this is always the situation for convex functions: the bad set has measure zero.
...
Here's the punchline of the entire book. I'm not being dramatic.
...
Why does this matter? It gives an intrinsic characterization of Sobolev spaces: $W^{1,p}$ functions are exactly the $L^p$ functions that agree with $C^1$ functions outside sets of small measure, with the right gradient control. It gets used in geometric measure theory to transfer smooth results to Sobolev functions. And in PDE it sometimes lets you reduce regularity arguments to the $C^1$ case.

Everything in Chapter 6 depends on the hard analysis from Chapters 3--5. The Sobolev embeddings, the BV theory, the fine properties of measures --- this is where those investments pay off. The $C^1$ approximation theorem is, I think, the crown jewel of the book. Six chapters of machinery, and this is what falls out at the end.
