...
attempt to explain it convinced me I didn't understand the definition as
well as I thought. So let's be careful.

The big idea: Lebesgue measure only ``sees'' $n$-dimensional things in
...
I keep forgetting the normalization. Writing it down one more time:
...
So as $\delta$ shrinks, we force covers to use smaller sets, which can
only make the infimum larger. Clean.
...
$+\infty$ to $0$. That value is the Hausdorff dimension.
...
dimension is $1$. That's special --- most sets aren't so nice.
...
directly but got confused by the covering argument. The upper bound is
...
optimal. I don't really get the intuition yet.
...
The highlight of this chapter. The isodiametric inequality via Steiner
symmetrization is one of the cleanest proofs I've seen all semester. I
...
Picture: you slide each vertical ``slice'' of $A$ so it's centered on the
hyperplane. Each slice keeps its length, so volume is preserved.
But the set gets ``rounder,'' so the diameter can only shrink. That's the
whole game.
...
\[
  ...
  \le (\diam A)^2.
\]
The point is that centering the intervals can only bring points in the
...
The second direction is basically just checking the normalization works
out. \qedhere
\end{proof}
...
basically a covering argument using the definition of $\mathcal{H}^s$.
...
structure. This definition felt arbitrary until I saw how it shows up in
rectifiability later.
\end{remark}
...
theme of the book. The main tool is Fubini applied to subgraphs.
...
The point for later: you can study integrability, level sets, and
...
That's Chapter~2. We've got Hausdorff measure, Hausdorff dimension, the
